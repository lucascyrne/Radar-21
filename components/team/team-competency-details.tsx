...
interface TeamCompetencyDetailsProps {
  teamResponses: TeamSurveyResponse[];
  showDetailed?: boolean;
  isLoading?: boolean;
}

// Mapeia questões para competências
...
export function TeamCompetencyDetails({
  teamResponses,
  showDetailed = false,
  isLoading = false,
}: TeamCompetencyDetailsProps) {
  if (isLoading) {
    return (
    ...
    return (
      <div className="flex items-center justify-center py-8">
        <p className="text-muted-foreground">
          Não há dados suficientes para exibir detalhes por competência.
        </p>
      </div>
    );
  }

  // Separar líderes de membros da equipe
  const leaders = teamResponses.filter((r) => r.role === "leader");
  const members = teamResponses.filter((r) => r.role === "member");

  // Calcular médias por competência
  const competencyDetails = Object.entries(competencyMap).map(
    ([questionId, topic]) => {
      // Pontuação média do líder
      const leaderScores = leaders
        .map(
          (l) => l[questionId as keyof TeamSurveyResponse] as number | undefined
        )
        .filter(
          (score): score is number => score !== undefined && score !== null
        );
      const userScore = leaderScores.length
        ? leaderScores.reduce((sum, score) => sum + score, 0) /
          leaderScores.length
        : 0;

      // Pontuação média da equipe (excluindo líderes)
      const teamScores = members
        .map(
          (m) => m[questionId as keyof TeamSurveyResponse] as number | undefined
        )
        .filter(
          (score): score is number => score !== undefined && score !== null
        );
      ...
      // Diferença entre equipe e líder
      const difference = teamAverage - userScore;
  ...
  const leaderAvg =
    competencyDetails.reduce((acc, item) => acc + item.userScore, 0) /
    competencyDetails.length;
  const teamAvg =
    competencyDetails.reduce((acc, item) => acc + item.teamAverage, 0) /
    competencyDetails.length;
          ...
          <div className="bg-white p-3 rounded-md border shadow-sm">
            ...
            <div
              className={`text-2xl font-bold ${
                overallDiff > 0 ? "text-green-600" : "text-red-600"
              }`}
            >
              {overallDiff > 0 ? "+" : ""}
              ...
              <div className="flex flex-wrap gap-2 sm:gap-4">
                <TooltipProvider>
                    ...
                    <TooltipTrigger asChild>
                      <div className="flex items-center gap-1">
                        <span className="text-blue-600 font-medium cursor-default">
                          Líder: {item.userScore.toFixed(1)}/5
                        </span>
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent className="max-w-xs">
                      <p>
                        Média das avaliações dos membros da equipe (excluindo o
                        líder).
                      </p>
                    </TooltipContent>
                  </Tooltip>
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent className="max-w-xs">
                      <p>
                        Diferença entre a média da equipe e a avaliação do
                        líder.
                      </p>
                      <p className="mt-1">
                        {item.difference > 0
            ...
            <div className="grid grid-cols-1 gap-4">
              ...
              <div className="space-y-1">
                <div className="flex justify-between text-sm font-medium">
                  ...
                  <span>{item.userScore.toFixed(1)}/5</span>
                </div>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {item.difference > 0 ? (
                  <Badge className="transition-none pointer-events-none bg-green-100 text-green-800 border-green-200 hover:bg-green-100">
                    Equipe avalia melhor que o líder (+
                    {item.difference.toFixed(1)})
                  </Badge>
                ) : item.difference < 0 ? (
                  <Badge className="transition-none pointer-events-none bg-red-100 text-red-800 border-red-200 hover:bg-red-100">
                    Líder avalia melhor que a equipe (
                    {item.difference.toFixed(1)})
                  </Badge>
                ) : (
                  <Badge className="transition-none pointer-events-none bg-gray-100 text-gray-800 border-gray-200 hover:bg-gray-100">
