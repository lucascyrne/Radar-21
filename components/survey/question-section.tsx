...
export function QuestionSection({
  ...
  const handleAnswerChange = async (questionId: string, value: string) => {
    try {
      ...
      const numericAnswers = Object.entries(newAnswers).reduce<SurveyResponses>(
        (acc, [key, val]) => {
      ...
      saveAnswers(numericAnswers).catch((error) => {
        ...
        toast.error("Erro ao salvar resposta", {
          description:
            "Sua resposta foi registrada localmente, mas houve um erro ao salvá-la. As respostas serão sincronizadas automaticamente.",
        });
      });
    } catch (error) {
            ...
            <FormItem className="space-y-3">
              ...
              >
                {likertOptions.map((option) => (
                  <div
                    key={option.value}
                    className="flex items-center space-x-2 space-y-0 mb-2"
                  >
                    <RadioGroupItem
                      value={option.value}
