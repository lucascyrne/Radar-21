...
export function ResultsTable({ userResults, teamResults, leaderResults }: ResultsTableProps) {
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead className="text-right">Diferença (Equipe - Líder)</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {data.map((row) => (
            <TableRow key={row.category}>
              ...
              <TableCell className="text-right">{row.user.toFixed(1)}</TableCell>
              <TableCell className="text-right">{row.team.toFixed(1)}</TableCell>
              <TableCell className="text-right">{row.leader.toFixed(1)}</TableCell>
